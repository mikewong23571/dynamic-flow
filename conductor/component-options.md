...
状态：已按用户授权完成 example 级安装与接入实验。最新证据、摩擦和推荐见 [Spike 结果](./tracks/component-integration-spike_20260919/results.md)。下文保留初始候选分析；实验安装不自动等于最终选型。

下文的初始分析基于设计稿与官方资料。现在已经验证的部分以 Spike 报告和 tech-stack.md 为准；未测试的替代方案仍只是候选。
...
当前首选：assistant-ui。用户明确表示它是较好的 Chat 选型；基于现有消息、流式交互与工具呈现能力，优先复用，不另建完整聊天组件。现已通过真实 Pi SDK + faux provider 接入验证；其余组件仍分别决定。
...
需要自研的衔接：提交时固定选中范围；映射消息／工具／中止状态；候选版本关联源码；“试运行”“采用方法”“保留结果”调用业务 API。这里需要业务衔接；本轮 example 已验证消息映射、选择快照和取消，但没有验证真实候选版本生成或业务采用 API。仅为使用该 UI，无需切换到 Next.js 或增加另一套 Agent loop。
...
本轮实验安装了 UI 和 Pi/Hono 候选，准确版本已核对；上述程序索引、完整执行机制与持久化尚未实验，不能把文档建议变成定论。
...
[技术栈](./tech-stack.md) 已同步本轮实测建议；后续正式实现须在单独 track 中推进。
