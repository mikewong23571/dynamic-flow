...
架构更新（2026-09-20）：本文保留实验时的结果与判断。下文“树优先、React Flow 延后、Monaco 作为核心编辑器、源码定位／AST 待验证”等建议已被 [当前设计](../../../docs/design.md) 取代：画布编辑同一份 JS 子集 IR，runtime 按节点执行；JS 只读查看。组件接入证据仍有效，但不能据此认定新 IR 编辑与执行闭环已经验证。
