...
更新：2026-09-20。当前原型设计，替代 [归档 v2](<./archive/Dynamic Workflow Workbench — 产品与技术设计 v2.md>)。正式 src 已实现 IR、画布编辑、有限节点 runtime、Pi 接入、文件保存及模型/工作库维护。本文件说明当前产品与实现边界，不代表 T1–T8 已验收；结论见 [当前 track 证据](../conductor/tracks/full-application_20260920/evidence.md)。
...
IR（Intermediate Representation，中间表示；本项目指工作流定义）是受约束的 JavaScript（JS）语义子集，用显式节点、参数和连接表达调用、顺序、条件分流与有限集合迭代。它可用 JS 模块中的声明式对象表示，由应用 runtime 按这些有限语义执行。
...
以下示例对应当前 Definition 形状；确切字段以 [共享记录类型](../src/shared/records.ts) 为准：
...
### 3.1 后续补充：输出契约与函数组合（待实现）

用户新增要求：Map / Reduce / Parallel 与输出 schema 必须一起考虑。后续按 [类型化数据流设计](typed-dataflow.md) 补齐端口输入输出契约、有限并发和画布表达。借鉴 Java 函数式签名，区分逐项转换、集合展开、筛选、整批汇总与严格归约；LLM 整批汇总不假定满足结合律。该文规定后续方向，前文示例和下文顺序执行描述仍反映现有实现，不能据此声称新能力已完成。
...
先复用 Pi 的 Agent loop、工具与取消，以及普通 JS 函数。剩余节点依赖与结果关联用薄适配实现；不重建通用 JS 解释器或编译器。后续具体反例若使这些语义需要大量框架代码，应先检验最小解决方案或评估现有 runtime，不为维持早期模块骨架削弱已授权故事。
...
第一版使用本地文件：每项工作一份 work.json，保存标题、目标、归档状态、材料、定义引用、布局、运行、结果、比较、消息与工具活动；不可变定义单独存为 JS 文件。暂不另建结果文件或追加事件文件。保留清楚的标识符关联；不同时维护文件与数据库两套权威状态。真实容量需要时再调整保存粒度或评估 SQLite。
...
模型设置（G1）由客户端表单与服务端设置文件共同实现。未保存时从 `.env.local`/进程环境获得初值，界面保存到 `data/model-settings.json` 后优先使用该文件。协议、端点、模型、API Key、推理强度、Temperature、Top P 和上下文窗口均可维护；各协议的真实限制由后端返回。密钥不回显，空白沿用现有值，保存只验证格式而不声称调用成功。

工作库维护（G2）提供搜索、分页、标题修改、归档及恢复。短标题与完整目标分开；作者可生成标题，用户手工维护后不被自动覆盖。归档保留原工作内容，不扩展为跨工作模板或发布平台。

## 8. 技术复用与范围裁剪

| 保留或采用方向                        | 用途                                               |
| ------------------------------------- | -------------------------------------------------- |
| React、TypeScript、Vite               | 正式客户端与构建入口                               |
| React Flow                            | 正式画布；节点/连接回写 IR，服务端校验端口与图结构 |
| 统一基础控件、Radix、Lucide、CSS 变量 | 控件语义与视觉；正式工作台不沿用 Spike 外观        |
| assistant-ui + Pi SDK                 | 自然语言编辑、Agent 节点执行；不重建 Agent loop    |
| Hono + SSE                            | 操作与运行事件                                     |
| react-resizable-panels                | 保留实验依赖，当前正式工作台未使用自由分栏         |
| React Markdown + GFM                  | 正式结果/报告与表格；TanStack Table 保留为实验依赖 |
...
用户已授权 [十九个核心故事及追加的 G1/G2](./user-stories.md)。正式实现已接线；以下持续用于检查关键链路，不代替 T1–T8 的逐项验收。

首个场景使用 3 条客户反馈：Agent 给出分类/汇总流程；用户在画布修改分类要求；选中样本运行；查看结果；再修改一个节点并比较；分别采用定义与保留结果；用选定结果生成最终报告。

正式前端参考研究与域内截图见 [客户端设计记录](../src/client/DESIGN-NOTES.md)；已有 Spike 的样式、配色和组件组合已被用户否定。代表性页面仍需要真实路线与用户视觉判断，技术测试通过不等于视觉验收。
...
发现具体反例时同步修正字段、调用双方与测试，不提前增加编译器、任意表达式系统或新的执行平台。
