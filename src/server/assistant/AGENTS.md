...
- [ ] P02/P12/P13/P26：实际生成/修改定义，固定目标，过期提案不覆盖；没有工具保存就不声称完成。
- [ ] P24/P08：逐协议真实验证文本、工具、错误和取消；记录端点/模型及限制，不记录密钥。
- [ ] P29：Pi 工具保存后、最后文字回复前，画布已经更新；联测到前端。
- [ ] P30：同名 toolCallId 不串会话/实例；输出缺必需字段如实失败。
