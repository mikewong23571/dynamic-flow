...
- [ ] 实例不复制职责节点；空集合零调用；编辑或换选择不影响冻结输入（场景 P03/P06/P07/P14）。
- [ ] 停止/失败/完成交错正确，保留成果，迟到结果不能继续下游；重试只有失败输入（场景 P08/P09）。
- [ ] 比较共用执行路径，不重复启动；停止基线后不执行候选（场景 P27/P28）。
- [ ] 真实 Pi 工具活动归属准确；与 files/client 核对进度和保存结果（场景 P30）。
