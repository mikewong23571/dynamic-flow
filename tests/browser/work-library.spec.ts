...
test('工作列表搜索、分页、重命名、归档恢复与重新打开', async ({
  ...
  await expect(library).toContainText('共 12 项工作');
  ...
  await expect(library.locator('.library-item')).toHaveCount(10);
  await library
    .getByRole('combobox', { name: '每页工作数', exact: true })
    .selectOption('20');
  ...
  await library
    ...
    .click();
  const dialog = page.getByRole('dialog', { name: '重命名工作', exact: true });
  const newTitle = `${prefix} 已整理`;
  await dialog
    .getByRole('textbox', { name: '工作标题', exact: true })
    .fill(newTitle);
  ...
  await renamedCard
    .getByRole('button', { name: `归档 ${newTitle}`, exact: true })
    .click();
  await expect(library).toContainText('共 11 项工作');
  await expect(
    library.getByRole('heading', { name: newTitle, exact: true }),
  ).toHaveCount(0);
  await library.getByRole('tab', { name: '已归档', exact: true }).click();
  await expect(library).toContainText('共 1 项工作');
  await expect(
    library.getByRole('heading', { name: newTitle, exact: true }),
  ).toBeVisible();
  await library
    .getByRole('button', { name: `恢复 ${newTitle}`, exact: true })
    .click();
  await expect(library).toContainText('共 0 项工作');
  await library.getByRole('tab', { name: '进行中', exact: true }).click();
  await expect(library).toContainText('共 12 项工作');
  expect(
  ...
  for (const control of [
    ...
    library.getByRole('button', { name: `归档 ${newTitle}`, exact: true }),
